...
int override_our_stratum_core() {
    char *file_name = "/etc/stratum_override.txt";
    ...
    // char *log_name = "/etc/stratum_override.log";
...
transmit_packet(NTP_Mode my_mode, /* The mode this machine wants to be */
    ...
    if (our_stratum < NTP_MAX_STRATUM) {
        our_stratum = override_our_stratum_core();
...
receive_packet(NCR_Instance inst, NTP_Local_Address *local_addr,
    ...
    if (logfileid != -1 && (log_raw_measurements || synced_packet)) {
        LOG_FileWrite(logfileid,
                      "%s %-15s %1c %2d %1d%1d%1d %1d%1d%1d %1d%1d%1d%d  %2d %2d %4.2f %10.3e %10.3e %10.3e %10.3e %10.3e %08"PRIX32" %1d%1c %1c %1c",
