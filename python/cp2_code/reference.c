...
REF_SetReference(int stratum, NTP_Leap leap, int combined_sources,
    ...
    our_stratum = override_our_stratum(); // stratum + 1;
    our_ref_id = ref_id;
    if (ref_ip)
        our_ref_ip = *ref_ip;
    else
...
REF_SetUnsynchronised(void) {
    ...
    our_stratum = override_our_stratum();
    are_we_synchronised = 0;
...
REF_EnableLocal(int stratum, double distance, int orphan) {
    ...
    local_stratum = override_our_stratum(); //CLAMP(1, stratum, NTP_MAX_STRATUM - 1);
